<div>
    <ul>
        @foreach ($conversations as $conversation)
            <li class="p-2 border-b relative {{  request()->route('intervention') ? (request()->route('intervention')->id == $conversation['intervention']->id ? 'bg-gray-200' : '') : '' }}">
                <a href="{{ route('interventions.chat', ['intervention' => $conversation['intervention']->id, 'user' => $conversation['user']->id]) }}" 
                    class="flex items-center"
                    wire:click="markAsRead({{ $conversation['intervention']->id }})">
                    ...
                    <div> 
                        <div class="font-bold" >
                            ...
                            @if($conversation['intervention']->statut_id == 3) Terminée @endif
